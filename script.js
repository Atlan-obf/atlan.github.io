...
window.addEventListener("load", () => {
    setTimeout(() => {
        ...
        playerInfo.style.animation = "fadeInPlayer 1s ease forwards";
    }, 2500); 
});
...
\`${playerName || "Unknown"}\`    \`${timeString}\``;
    ...
    const embed = { description: description, color: 0xD1759B };

    fetch(webhookURL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: "Menu Logger", embeds: [embed] })
